/***************************************************************************//**
  ...
  и тип @c t_lclock_ticks для представления текущего значения системного счетчика.
  ...
  Все функции принимают значения в отсчетах системного клока, поэтому необходимо
  использовать константу #LCLOCK_TICKS_PER_SECOND для времени в количество отсчетов.
  Также можно использовать макросы LTIMER_MS_TO_CLOCK_TICKS() и LTIMER_CLOCK_TICKS_TO_MS()
  для перевода интервалов в миллисекундах в количество тиков и наоборот.

  Для работы с модулем в проекте нужно добавить в проект в стандартные пути 
  для включения заголовочных файлов пути к файлу ltimer.h и к порту директории с портом
  ...
  можно с помощью ltimer_expired(). Количество отсчетов системного таймера
